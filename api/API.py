...
class SmiteAPI:
    ...
    def generate_signature(self, method):
        combined = self.API_DEVID + method + self.API_AUTHKEY + self.get_timestamp()
    ...
    def create_session(self):
        ...
        if root.find(self.NAMESPACE+"ret_msg").text == "Approved":
            ...
            timestamp = self.xml_to_datetime(root.find(self.NAMESPACE+"timestamp").text)
    ...
    def web_request(self, method, args):
        ...
        if args is not None: url_segments = url_segments + args
        the_url = '/'.join(url_segments)
        self.log(the_url)
        print the_url
        response = None
        try:
            response = urllib2.urlopen(the_url.encode('utf-8'))
    ...
    def data_usage(self):
        ...
        usage = {
            "active_sessions": resp_tree.find(self.NAMESPACE + "Active_Sessions").text,
            "concurrent_sessions": resp_tree.find(self.NAMESPACE + "Concurrent_Sessions").text,
            "request_limit_daily": resp_tree.find(self.NAMESPACE + "Request_Limit_Daily").text,
            "session_limit_daily": resp_tree.find(self.NAMESPACE + "Session_Cap").text,
            "session_time_limit": resp_tree.find(self.NAMESPACE + "Session_Time_Limit").text,
            "total_requests_today": resp_tree.find(self.NAMESPACE + "Total_Requests_Today").text,
            "total_sessions_today": resp_tree.find(self.NAMESPACE + "Total_Sessions_Today").text,
        }
        return usage
